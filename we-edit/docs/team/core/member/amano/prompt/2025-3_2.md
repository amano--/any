...
突然すいません。Bチームの生産性の向上方法について、提案というか相談があるので時間をいただけないでしょうか。多分一時間くらいかかる可能性があるのでそのくらいの時間を見積もっていただければ

### Bチーム改善案

#### 現状

Reactのベストプラクティスへの理解不足、テスタブルに作る意識に欠けてるので、それ起因で諸問題が発生している。
→ベストプラクティス、テスタブルな作成方法について知見を深める必要がある

#### 対策

単体テストの自動生成を通してReactのベストプラクティスへの理解を深める

単体テストに何を書くべきか
e2eテストに何を書くべきかの模索

既存のコードをどうリファクタリングするのが良いか?
コードをテスタブルに書くにはどうしたらいいか?

に頭脳を使うことに集中。コードはAIに任せる。

#### 作業

既存のコードから単体テストを作成
AIに問題点を分析させ、資料をよみ理解を深める

Container/Presentationalパターンを適用し、Presentationalとhooksに分離し、再度テストを自動生成しテストの作られ方の違いについて知見を深める。

[コンテナ・プレゼンテーションパターン](https://zenn.dev/morinokami/books/learning-patterns-1/viewer/presentational-container-pattern)

笠原さんからe2eのテスト作成方法を学ぶ

各自e2eのテスト作成

みんなで、単体のテスト作成方針、e2eのテスト作成方針を話し合う
テスト周りの基本方針を定める
...
最終確認に、npm run build(lintチェックを兼ねて) npm run start
→ Docker create-repository.shからの解放

### 平井さん

平井さんに単体テスト作成方法に習熟してもらう
→　バックエンドに布教
→　別チームのフロントにも布教

### ペアプロの使い方改善

検証、探索、考察、方針決め、学習、教育等はペアプロ向き
各自AIを教師兼ドライバーとして、それぞれペアプロする感じなるはず

午後11-12 3-4 みたいにペアプロ時間を設定するとか?
...
→各自Work
→お互い作ったコードを相互レビュー
→ どちらかを採用するか、いいとこ取りして再作成をする等してApprove

### 要望

バックエンドの言語が変わる可能性は0 ?

しばらくタスク割り振りと優先順位決めをできる権限を与えて欲しい。

AIを活用したコスト削減的な　PoC的なプロジェクトが立ち上がったら、引っ張ってきてくれると嬉しい。
仮説検証、要求分析からコーディングまでをエンジニアチームが主導できるプロジェクトに参加したい。
